...
/* Handle request to insert image in certh infrastructure */
app.post(
    "/certh/insertImageByUrl",
    async function(req: express.Request, res: express.Response) {
        try {
            const imageUrl = decodeURIComponent(req.body.imageUrl);
            if (imageUrl) {
                const id = await insertImageinCerth(imageUrl);
                const result = await checkJobCerth(id);
                console.log(result);
                res.send({ staus: 1, message: result })
                // TODO: insert image in BD
            } else {
                res.send({ status: 2, message: "Invalid image url or alt text." });
            }
        } catch (err) {
...
/* Handle request to search image in certh infrastructure */
app.post(
    "/certh/searchImageByUrl/",
    async function(req: express.Request, res: express.Response) {
        try {
            const imageUrl = decodeURIComponent(req.body.imageUrl);
            if (imageUrl) {
                const jobId = await insertImageinCerth(imageUrl);
                const result = await checkJobCerth(jobId);
                const imageId = result.id;
                const match = await searchImageCerth(imageId);
                if (match === -1) {
                  await deleteImageCerth(imageId);
                  res.send({ status: 3, message: "Error searching for image." });
                } else if (match) {
                  await deleteImageCerth(imageId);
                  res.send({ status: 1, message: match });
                } else {
                  res.send({ status: 5, message: "No matching image found" });
                }
            } else {
                res.send({ status: 2, message: "Invalid image url or alt text." });
            }
...
app.post("**", function(_, res: express.Response) {
    res.send("Invalid service");
});
